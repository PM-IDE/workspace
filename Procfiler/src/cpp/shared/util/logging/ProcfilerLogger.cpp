...
char* PatchMessage(const std::string& message) {
    const auto patchedMessage = new char[message.length() + 1];
    auto i = 0;
...
void ProcfilerLogger::LogInformation(const std::string& message) const {
    if (!myIsEnabled) return;

    const auto patchedMessage = PatchMessage(message);
...
ProcfilerLogger::ProcfilerLogger() {
    const auto enableLoggingEnv = std::getenv(enableConsoleLoggingEnv.c_str());
    myIsEnabled = enableLoggingEnv != nullptr && std::string(enableLoggingEnv) == "1";
}

void ProcfilerLogger::LogError(const std::string &message) const {
    if (!myIsEnabled) return;
