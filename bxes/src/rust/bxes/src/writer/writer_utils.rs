...
pub fn try_write_system_metadata(system_metadata: &SystemMetadata, context: Rc<RefCell<BxesWriteContext>>) -> Result<(), BxesWriteError> {
...
pub fn try_write_variants(log: &BxesEventLog, context: Rc<RefCell<BxesWriteContext>>) -> Result<(), BxesWriteError> {
  write_collection_and_count(context.clone(), false, log.variants.len() as u32, || {
    for variant in &log.variants {
      try_write_u32_no_type_id(context.borrow_mut().writer.as_mut().unwrap(), variant.traces_count)?;
...
pub fn try_write_event(event: &BxesEvent, context: Rc<RefCell<BxesWriteContext>>) -> Result<(), BxesWriteError> {
  let exists = context.borrow().values_indices.borrow().contains_key(&event.name);

  if exists {
    let index = *context.borrow().values_indices.borrow().get(&event.name).unwrap();
  ...
  try_write_i64_no_type_id(context.borrow_mut().writer.as_mut().unwrap(), event.timestamp)?;
  ...
  write_collection_and_count(context.clone(), false, count(properties), || {
    if let Some(properties) = properties {
      for property in properties {
        try_write_kv_index(context.clone(), &(property.0.clone(), property.1.clone()), false)?;
      }
    }
  ...
  write_collection_and_count(context.clone(), false, count(globals), || {
    if let Some(globals) = globals {
      for global in globals {
        try_write_enum_value_no_type_index(context.borrow_mut().writer.as_mut().unwrap(), &global.entity_kind)?;

        write_collection_and_count(context.clone(), false, global.globals.len() as u32, || {
          for global in &global.globals {
            try_write_kv_index(context.clone(), &(global.0.clone(), global.1.clone()), false)?;
          }
...
pub fn try_write_kv_index(
  ...
  if !context.borrow().kv_indices.borrow().contains_key(kv) {
    Err(BxesWriteError::FailedToFindKeyValueIndex((kv.0.clone(), kv.1.clone())))
  } else {
...
pub fn try_write_extensions(context: Rc<RefCell<BxesWriteContext>>, extensions: Option<&Vec<BxesExtension>>) -> Result<(), BxesWriteError> {
...
pub fn try_write_attributes(
  ...
  write_collection_and_count(context.clone(), write_leb_128_count, count(attributes), || {
    if let Some(attributes) = attributes {
      for (key, value) in attributes {
        try_write_kv_index(context.clone(), &(key.clone(), value.clone()), write_leb_128_count)?;
      }
    }
...
pub fn try_write_key_values(log: &BxesEventLog, context: Rc<RefCell<BxesWriteContext>>) -> Result<(), BxesWriteError> {
    ...
    execute_with_kv_pairs(log, |value| {
      match value {
        ...
        ValueOrKeyValue::KeyValue((key, value)) => {
          let exists = context.borrow().kv_indices.borrow().contains_key(&(key.clone(), value.clone()));

          if !exists {
            let count = context.borrow().kv_indices.borrow().len();
            let key_index = *context.borrow().values_indices.borrow().get(key).unwrap();
            let value_index = *context.borrow().values_indices.borrow().get(value).unwrap();

            try_write_leb_128(context.borrow_mut().writer.as_mut().unwrap(), key_index as u32)?;

            try_write_leb_128(context.borrow_mut().writer.as_mut().unwrap(), value_index as u32)?;
...
pub fn try_write_values(log: &BxesEventLog, context: Rc<RefCell<BxesWriteContext>>) -> Result<(), BxesWriteError> {
  write_collection_and_count_after(context.clone(), || {
    execute_with_kv_pairs(log, |value| {
      match value {
...
fn try_write_value(context: &mut BxesWriteContext, value: &BxesValue) -> Result<(), BxesWriteError> {
  match value {
    ...
    BxesValue::StandardLifecycle(value) => try_write_standard_lifecycle(context.writer.as_mut().unwrap(), value),
    ...
    BxesValue::SoftwareEventType(value) => try_write_software_event_type(context.writer.as_mut().unwrap(), value),
  }
}

pub fn try_write_software_event_type(writer: &mut BinaryWriter, value: &SoftwareEventType) -> Result<(), BxesWriteError> {
...
pub fn try_write_artifact(context: &mut BxesWriteContext, artifact: &BxesArtifact) -> Result<(), BxesWriteError> {
  ...
  try_write_u8_no_type_id(context.writer.as_mut().unwrap(), get_type_id_byte(TypeIds::Artifact))?;

  try_write_u32_no_type_id(context.writer.as_mut().unwrap(), artifact.items.len() as u32)?;
...
pub fn try_write_drivers(context: &mut BxesWriteContext, drivers: &BxesDrivers) -> Result<(), BxesWriteError> {
  ...
  try_write_u8_no_type_id(context.writer.as_mut().unwrap(), get_type_id_byte(TypeIds::Drivers))?;

  try_write_u32_no_type_id(context.writer.as_mut().unwrap(), drivers.drivers.len() as u32)?;
...
pub fn try_write_f64_no_type_id(writer: &mut BinaryWriter, value: f64) -> Result<(), BxesWriteError> {
...
pub fn try_write_lifecycle(writer: &mut BinaryWriter, lifecycle: &Lifecycle) -> Result<(), BxesWriteError> {
  match lifecycle {
    Lifecycle::Braf(braf_lifecycle) => try_write_braf_lifecycle(writer, braf_lifecycle),
    Lifecycle::Standard(standard_lifecycle) => try_write_standard_lifecycle(writer, standard_lifecycle),
  }
}

pub fn try_write_braf_lifecycle(writer: &mut BinaryWriter, value: &BrafLifecycle) -> Result<(), BxesWriteError> {
  try_write_enum_value(writer, &TypeIds::BrafLifecycle, value)
}

fn try_write_enum_value_no_type_index<T: ToPrimitive>(writer: &mut BinaryWriter, value: &T) -> Result<(), BxesWriteError> {
  try_write(|| writer.write_u8(T::to_u8(value).unwrap()))
}

fn try_write_enum_value<T: ToPrimitive>(writer: &mut BinaryWriter, type_id: &TypeIds, value: &T) -> Result<(), BxesWriteError> {
  try_write(|| {
...
pub fn try_write_standard_lifecycle(writer: &mut BinaryWriter, value: &StandardLifecycle) -> Result<(), BxesWriteError> {
...
fn try_write(mut write_func: impl FnMut() -> crate::binary_rw::core::Result<usize>) -> Result<(), BxesWriteError> {
  match write_func() {
    Ok(_) => Ok(()),
