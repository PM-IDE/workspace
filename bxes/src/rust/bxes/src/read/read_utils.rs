...
pub fn try_read_event_log_metadata(context: &mut ReadContext) -> Result<BxesEventLogMetadata, BxesReadError> {
...
fn try_read_value_attributes(context: &mut ReadContext) -> Result<Option<Vec<ValueAttributeDescriptor>>, BxesReadError> {
...
pub fn try_read_classifiers(context: &mut ReadContext) -> Result<Option<Vec<BxesClassifier>>, BxesReadError> {
...
pub fn try_read_globals(context: &mut ReadContext) -> Result<Option<Vec<BxesGlobal>>, BxesReadError> {
  let count = try_read_u32(context.reader.as_mut().unwrap())?;
  if count == 0 {
    Ok(None)
  } else {
...
pub fn try_read_extensions(context: &mut ReadContext) -> Result<Option<Vec<BxesExtension>>, BxesReadError> {
  let count = try_read_u32(context.reader.as_mut().unwrap())?;
  if count == 0 {
    Ok(None)
  } else {
...
pub fn string_or_err(value: &BxesValue) -> Result<Arc<str>, BxesReadError> {
...
pub fn try_read_traces_variants(context: &mut ReadContext) -> Result<Vec<BxesTraceVariant>, BxesReadError> {
...
pub fn try_read_trace_variant(context: &mut ReadContext) -> Result<BxesTraceVariant, BxesReadError> {
...
pub fn try_read_trace_variant_metadata(context: &mut ReadContext) -> Result<Vec<(Arc<BxesValue>, Arc<BxesValue>)>, BxesReadError> {
...
pub fn try_read_trace_variant_events(context: &mut ReadContext) -> Result<Vec<BxesEvent>, BxesReadError> {
...
fn try_read_event_attributes(context: &mut ReadContext) -> Result<Option<Vec<(Arc<BxesValue>, Arc<BxesValue>)>>, BxesReadError> {
  ...
  if value_attrs_len > 0 {
    for i in 0..value_attrs_len {
      ...
      if value_type_id != null_type_id || descriptor.type_id == null_type_id {
        ...
        attributes.as_mut().unwrap().push((key, Arc::new(value)));
      }
    }
  }
...
pub fn try_fill_attributes(
  ...
  leb_128: bool,
  attributes: &mut Option<Vec<(Arc<BxesValue>, Arc<BxesValue>)>>,
...
fn try_read_attributes(context: &mut ReadContext, leb_128: bool) -> Result<Option<Vec<(Arc<BxesValue>, Arc<BxesValue>)>>, BxesReadError> {
  let attributes_count = try_read_count(context, leb_128)?;
...
fn try_read_kv_pair(context: &mut ReadContext, leb_128: bool) -> Result<(Arc<BxesValue>, Arc<BxesValue>), BxesReadError> {
...
pub fn try_read_values(context: &mut ReadContext) -> Result<(), BxesReadError> {
  ...
  for _ in 0..values_count {
    let value = try_read_bxes_value(context)?;
    context.metadata.values.as_mut().unwrap().push(Arc::new(value));
  }
...
fn try_read_bxes_value(context: &mut ReadContext) -> Result<BxesValue, BxesReadError> {
  ...
  match try_read_type_id(reader)? {
    ...
    TypeIds::String => Ok(BxesValue::String(Arc::from(try_read_string(reader)?))),
    ...
    TypeIds::SoftwareEventType => Ok(BxesValue::SoftwareEventType(try_read_software_event_type(reader)?)),
  }
}
...
pub fn try_read_artifact_item(context: &mut ReadContext) -> Result<BxesArtifactItem, BxesReadError> {
...
pub fn try_read_software_event_type(reader: &mut BinaryReader) -> Result<SoftwareEventType, BxesReadError> {
...
fn try_read_bytes(reader: &mut BinaryReader, length: usize) -> Result<Vec<u8>, BxesReadError> {
  ...
  match reader.read(&mut buf) {
    Ok(_) => Ok(buf),
    Err(err) => Err(BxesReadError::FailedToReadValue(FailedToReadValueError::new(
      offset,
      err.to_string(),
    ))),
  }
...
fn try_read<T>(reader_pos: usize, mut read_func: impl FnMut() -> crate::binary_rw::core::Result<T>) -> Result<T, BxesReadError> {
  match read_func() {
    Ok(value) => Ok(value),
    Err(err) => Err(BxesReadError::FailedToReadValue(FailedToReadValueError::new(
      reader_pos,
