...
pub fn lossy_count_test_3() {
    execute_streaming_counter_test(
        vec![1, 1, 2, 3, 1, 2, 1, 2, 2, 1,1, 2, 2, 1, 1, 2, 1, 2, 2, 2, 2, 1, 1,1, 1, 1, 2, 2, 1, 2, 1],
...
fn execute_streaming_counter_test<T: Hash + Eq + Clone + Ord + Debug, TCounter: StreamingCounter<T>>(
    ...
    assert_eq!(
        expected_result.into_iter().map(|expected_freq| (expected_freq.0, expected_freq.1.to_string())).collect::<Vec<(T, String)>>(),
        frequencies.iter().map(|e| (e.key().clone(), e.approx_frequency().to_string())).collect::<Vec<(T, String)>>()
    );
}