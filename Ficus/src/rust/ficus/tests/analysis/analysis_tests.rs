...
use ficus::features::analysis::{
  entropy::{
    ...
    pos_entropy::calculate_pos_entropies,
    pos_entropy_fast::calculate_pos_entropies_fast,
  },
  log_info::{
...
fn test_dfg_info() {
  ...
  assert_eq!(followers.get("B").unwrap(), &2usize);

  let followers = dfg.get_followed_events(&"B".to_string()).unwrap();
  assert_eq!(followers.get("C").unwrap(), &2usize);
