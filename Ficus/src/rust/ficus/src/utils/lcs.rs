...
impl<'a, T> LCSSearchResult<'a, T> {
  pub fn lcs(&self) -> &Vec<&'a T> {
    self.lcs.as_ref()
  }

  pub fn first_indices(&self) -> &Vec<usize> {
    self.indices_in_first_sequence.as_ref()
  }
  pub fn second_indices(&self) -> &Vec<usize> {
    self.indices_in_second_sequence.as_ref()
  }
}
