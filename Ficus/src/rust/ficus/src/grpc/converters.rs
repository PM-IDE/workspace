...
use crate::features::discovery::ocel::graph_annotation::{NodeObjectsState, OcelAnnotation, ProcessNodesStates};
...
fn convert_to_grpc_ocel_data(data: &OcelData) -> GrpcOcelData {
  GrpcOcelData {
    ...
    object_type: data.object_type().to_owned(),
    action: Some(match data.action() {
      OcelObjectAction::Allocate => grpc_ocel_data::Action::Allocate(()),
      OcelObjectAction::Consume => grpc_ocel_data::Action::Consume(()),
      OcelObjectAction::AllocateMerged(ids) => grpc_ocel_data::Action::MergedObjectAllocation(
        GrpcMergedObjectAllocation {
          consumed_objects_ids: ids.clone()
        }
      ),
      OcelObjectAction::ConsumeWithProduce(ids) => grpc_ocel_data::Action::ProduceObjectConsumption(
        GrpcProduceObjectConsumption {
          produced_objects_ids: ids.clone()
        }
      ),
    }),
  }
