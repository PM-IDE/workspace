...
impl<TKey: Hash + Eq, TValue> SlidingWindowProcessor<TKey, TValue> {
    ...
    pub fn new_time(invalidation_duration: Duration) -> Self {
        Self {
            storage: HashMap::new(),
            invalidator: Rc::new(Box::new(move |_, stamp| {
                match stamp.add(invalidation_duration) < Utc::now() {
    ...
    pub fn invalidate(&mut self) {
        ...
        self.storage.retain(|_, value| invalidator(&value.value, &value.timestamp) == InvalidationResult::Retain)
    }
}