...
fn transfer_data_from_event_to_node_user_data(data: &UserDataImpl, user_data_impl: &mut UserDataImpl, belongs_to_root_sequence: bool) {
    ...
    for (event_index, event) in trace.iter().enumerate() {
      ...
      if let Some(pattern_kind) = pattern_kind {
        ...
        event
          .borrow_mut()
          .user_data_mut()
          .put_concrete(NODE_UNDERLYING_PATTERNS_INFOS_KEY.key(), patterns);
      }
    }
  }
}
...
fn transfer_vector_like_user_data<T: Clone>(data: &UserDataImpl, key: &DefaultContextKey<Vec<T>>, user_data_impl: &mut UserDataImpl) {
...
fn adjust_log_user_data(log: &XesEventLogImpl) {
  for (trace_index, trace) in log.traces().iter().enumerate() {
    for (event_index, event) in trace.borrow().events().iter().enumerate() {
      ...
      event.borrow_mut().user_data_mut().put_concrete(
        NODE_CORRESPONDING_TRACE_DATA_KEY.key(),
        vec![NodeAdditionalDataContainer::new(CorrespondingTraceData::new(false), coordinates)],
      );
