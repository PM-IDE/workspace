  ...
  TLog: EventLog,
{
  ...
  let dfr_calculator = |first: &Arc<str>, second: &Arc<str>, log_info: &dyn EventLogInfo| {
  ...
  let dpr_calculator = |first: &Arc<str>, second: &Arc<str>, log_info: &dyn EventLogInfo| {
...
pub fn calculate_default_dfg_entropy<TLog>(log: &TLog, ignored_events: Option<&HashSet<Arc<str>>>) -> HashMap<String, f64>
where
  TLog: EventLog,
{
  let dfr_calculator = |first: &Arc<str>, second: &Arc<str>, log_info: &dyn EventLogInfo| {
...
where
  TDfrEntropyCalculator: Fn(&Arc<str>, &Arc<str>, &dyn EventLogInfo) -> f64,
  TDprEntropyCalculator: Fn(&Arc<str>, &Arc<str>, &dyn EventLogInfo) -> f64,
{
  let mut entropy = HashMap::new();
  let events_names = &log_info.all_event_classes();

  let mut dfr_events_names = events_names.clone();
  let fake_end = Arc::<str>::from(FAKE_EVENT_END_NAME);
  dfr_events_names.push(&fake_end);

  let mut dpr_events_names = events_names.clone();
  let fake_start = Arc::<str>::from(FAKE_EVENT_START_NAME.to_string());
